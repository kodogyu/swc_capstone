...
void Visualizer::displayPosesWithKeypoints(const std::vector<Eigen::Isometry3d> &poses, const std::vector<cv::Mat> &keypoints_3d_vec) {
    pangolin::CreateWindowAndBind("Visual Odometry Example", 1024, 768);
    glEnable(GL_DEPTH_TEST);
    glEnable(GL_BLEND);
    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
...
void Visualizer::displayPosesAnd3DPoints(const std::vector<Eigen::Isometry3d> &poses, const std::vector<Eigen::Vector3d> &points_3d) {
pangolin::CreateWindowAndBind("display Poses And 3D Points Window", 1024, 768);
    glEnable(GL_DEPTH_TEST);
    glEnable(GL_BLEND);
    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
    ...
    while (!pangolin::ShouldQuit()) {
        ...
        glPointSize(5.0f);
        glBegin(GL_POINTS);
        for (Eigen::Vector3d point : points_3d) {

            glColor3f(colors[0][0], colors[0][1], colors[0][2]);  // red points
